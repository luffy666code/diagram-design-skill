...
**Reference and font boundary:** Diagrams must work offline, so use locally available or bundled fonts from the current style guide. Do not search websites online for visual inspiration or brand tokens unless the user explicitly requests website-based onboarding. Files under `assets/` are layout and composition references only; never inherit their colors, typography, or style tokens. Always use the current values defined in [`references/style-guide.md`](references/style-guide.md).
...
- Embedded CSS only; no external CSS, font, image, or script dependencies
