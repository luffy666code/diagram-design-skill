...
2. Trace each family to its source: an installed/system stack, a bundled font asset, or a custom `@font-face` that can be packaged locally.
3. If the family is locally available or bundled, carry the exact family name and weights into the style guide and generated HTML without adding remote stylesheet URLs. Preserve an intentional system stack in order and verify the resolved family on the target machine.
4. A custom-hosted or paid font is not compatible with the default offline output unless the user separately approves and packages the font locally. Label that role `fallback` unless it is available offline; never silently add a remote font URL or claim an exact match.
