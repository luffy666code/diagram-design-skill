...
3. Make it standalone:
   ...
   - Do not inject remote font imports. Exports must remain offline-safe and inherit the local or bundled font stacks already declared by the source diagram.
...
- **Source is missing fonts at runtime**: Playwright will substitute, and the screenshot may look off. Check that the source HTML uses the local or bundled font stack from `references/style-guide.md`; fix the source rather than working around it in export.
